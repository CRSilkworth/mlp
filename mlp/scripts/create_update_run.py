...
from mlp.utils.docker import build_image, push_image, local_image_exists, remote_image_exists
...
def check_environment_set():
  ...
  for environ in environs:
    if os.environ.get(environ) is None or os.environ.get(environ).strip() == '':
      sys.exit('{environ} not set. Must set to appropriate value before runnign this script.'.format(environ=environ))
...
def create_update_run(
  ...
  if remote_image_exists(image_name, vc.version):
    ...
    if exit_code != 0:
      sys.exit("Failed to push image {image_name}:{image_tag}. Try pushing yourself with 'docker push {image_name}:{image_tag} to see what's wrong.".format(image_name=image_name, image_tag=vc.version))
  ...
  experiments = [d.name for d in client.list_experiments().experiments]
  if experiment not in experiments:
    client.create_experiment(experiment)
  experiment_id = client.get_experiment(experiment_name=experiment).id

  # Upload pipeline if it doesn't exist
  pipelines = [d.name for d in client.list_pipelines().pipelines]
